...
public abstract class NetFancyBehaviour : NetworkBehaviour
{
    ...
    protected void RaiseLocalEvent<T>(ref T eventData, bool broadcast = false) where T : struct
    {
    ...
    protected void RaiseLocalEvent<T>(bool broadcast = false) where T : struct
    {
        var eventData = default(T);
        _eventSys.RaiseLocalEvent(gameObject, ref eventData);
